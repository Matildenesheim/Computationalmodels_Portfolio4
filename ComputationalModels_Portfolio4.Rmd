...
Discuss the role that meta-analytic priors should have in scientific practice. Should we systematically use them?
...
```{r}
library(pacman)
p_load(tidyverse, rethinking, brms, gdata, readxl, brmstools, patchwork, dplyr, stringr)
 setwd("~/Desktop/OneDrive/Cognitive Science/4 semester/Computational Models")

#devtools::install_github("mvuorre/brmstools")
library(brmstools)

dfP = read.xls("Assignment4PitchDatav2.xlsx")
df = read.xls("Assignment4MetaData.xlsx")
...
#same but in ggplot
#ggplot(p_sum, aes(x = SDScale, color = Diagnosis)) + 
  geom_density()
...
#plotting predictions
b4 = gggplot(m4, x = 'Diagnosis', y = 'SDScale', dataframe = p_sum) +
...
#plot estimates for the model with the meta-analytic prior. 
b5 = gggplot(mMeta, x = 'Diagnosis', y = 'SDScale', dataframe = p_sum) +
  ggtitle("Prior for beta_diagnosis = (0,0.24)")
...
#plotting betas against each other
#plotting betas against each other
sim1control = sim(mMeta, data = p_sum[p_sum$Diagnosis == '0',])
sim1schizo = sim(mMeta, data = p_sum[p_sum$Diagnosis == '1',])
...
# WAIC difference 
compare(m1, m2, m3, m4, mMeta) #all values from different models are so alike/close to each other, so we can't really use WAIC for anything.

# Beta difference, comparing the posteriors 
...
postMeta = extract.samples(mMeta)

post_df = rbind(post1$b, post2$b, post3$b, post4$b, postMeta$b)
